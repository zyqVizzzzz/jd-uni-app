	...
	<view class="data-page">
			...
			<view class="data-row">
				...
				<view class="data-item">
					...
					<text class="value orange">{{
						formatDuration(latestRecord.duration)
					}}</text>
				</view>
			</view>
			...
			<view class="data-row">
				<view class="data-item">
					...
					<text class="value blue">{{
						formatPace(latestRecord.averagePace)
					}}</text>
				</view>
				<view class="data-item">
		...
		<view class="flex gap-4 mt-4">
			...
			<view class="flex-1 bg-white rounded-2xl p-4">
				...
				<text class="block text-base mt-2">{{
					formatDate(latestRecord.date)
				}}</text>
				<text class="block text-2xl font-bold mt-8"
					>{{ getDaysAgo(latestRecord.date) }} 天前</text
				>
			</view>
			...
			<view class="flex-1 bg-white rounded-2xl p-4">
				<view class="mb-6">
					...
					<text class="block text-2xl font-bold text-blue-400"
						>{{ totalStats.totalDistance || 0 }}M</text
					>
				</view>
				<view>
					<text class="block text-base text-gray-700">总训练时长</text>
					<text class="block text-2xl font-bold text-yellow-400">{{
		...
		<view class="bg-white rounded-2xl p-4 mt-4">
			<view class="flex justify-between mb-4">
				<view>
					<text class="block text-base text-gray-700"
						>{{ currentMonth }}月游了</text
					>
					<text class="block text-2xl font-bold text-yellow-400"
						>{{ monthlyTarget.currentDistance }}M</text
					>
				</view>
				<view class="text-right">
					<text class="block text-base text-gray-700"
						>{{ currentMonth }}月目标</text
					>
					<text class="block text-2xl font-bold text-yellow-400"
						>{{ (monthlyTarget.completionRate * 100).toFixed(0) }}%</text
					>
				</view>
			</view>
			...
			<!-- 进度条 -->
			<view
				class="relative h-8 bg-gray-100 rounded-full mt-4 progress-container"
			>
				<view
					class="progress-fill"
					:style="{ width: `${monthlyTarget.completionRate * 100}%` }"
				></view>
				<view class="progress-text">
					<text class="text-sm text-gray-600"
						>{{ monthlyTarget.currentDistance }}/{{
							monthlyTarget.targetDistance
						}}</text
					>
				</view>
			</view>
		...
		<view class="add-record-btn" @tap="handleMockData">
			<text class="btn-text">模拟游泳记录</text>
		</view>
